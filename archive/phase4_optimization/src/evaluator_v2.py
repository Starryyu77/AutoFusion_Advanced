...
class RealDataFewShotEvaluator(BaseEvaluator):
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        self.config = config  # Phase 4: Save config for early stopping and time limit
    ...
    def _few_shot_train(self, model: nn.Module, train_loader) -> Dict[str, float]:
        ...
        max_training_time = self.config.get('max_training_time', None)  # seconds
        ...
        for epoch in range(self.train_epochs):
            ...
            # Phase 4: Check time limit
            if max_training_time and start_time:
                elapsed = time.time() - start_time
                if elapsed > max_training_time:
                    logger.info(f"Training stopped: time limit exceeded ({elapsed:.1f}s > {max_training_time}s)")
                    break

            # Phase 4: Check early stopping (using training accuracy as proxy)
